...
const Income = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post("/incomes/", { amount: parseFloat(form.amount), date });
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: "24px" }}>
        <h3 style={{ fontSize: "15px", fontWeight: "600", marginBottom: "18px" }}>Add New Income</h3>
              ...
              {incomes.map((inc) => (
                <tr key={inc.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  <td style={{ padding: "13px 0", color: "var(--text-muted)" }}>
                    {formatMonth(inc.date)}
                  </td>
                  <td style={{ padding: "13px 0", textAlign: "right", fontWeight: "600", color: "var(--success)" }}>
